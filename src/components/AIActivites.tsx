      ...
      <div className="max-w-7xl mx-auto px-4 flex flex-col">
        ...
        <div className="flex flex-wrap justify-center gap-5 px-3">
          {activities.map((activity) => (
            <Card
              key={activity.id}
              className={`group overflow-hidden ${activity.bgColor} shadow-xl hover:shadow-2xl transition-all duration-500 transform hover:-translate-y-2 border-0 rounded-2xl p-0 
      w-full sm:w-[48%] m-0`}
            >
              <h1 className="text-lg text-white font-bold text-center flex items-center justify-center mt-3">
                {activity.title}
              </h1>
